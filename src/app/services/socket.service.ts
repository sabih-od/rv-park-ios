...
export class SocketService {
  ...
  constructor(private network: NetworkService, public events: EventsService) {
    this.socket = new WebSocket(
      'wss://kjrvportal.com:3001/app/67e75c5e-40ba-4a6d-88bc-587dd7a1a8fe?protocol=7&client=js&version=8.0.1&flash=false'
    );
  ...
  getChannelId(params) {
    return new Promise(async (resolve) => {
      if (this.socketId) {
        const res = await this.network.chatChannelCreate(
          params.user_id,
          params.id
        );
        console.log(res);
